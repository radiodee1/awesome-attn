...
ONMT="OpenNMT-py" #"$( cd "$( dirname "${BASH_SOURCE[0]}" )/.." && pwd )"
...
DATA="data"
TRAIN_SRC=$DATA/*train.from
TRAIN_TGT=$DATA/*train.to
VALID_SRC=$DATA/*valid.from
VALID_TGT=$DATA/*valid.to
TEST_SRC=$DATA/*test.from
TEST_TGT=$DATA/*test.to

BPE="" # default
BPE="src+tgt" # "src" # src, tgt, src+tgt
...
function lines_check {
    l1=`wc -l $1`
    l2=`wc -l $2`
    l1=`echo $l1 | grep -oP '^\S*'`
    l2=`echo $l2 | grep -oP '^\S*'`
    if [[ $l1 != $l2 ]]; then
        echo "$1 = $l1 , $2 = $l2"
...
if [[ "$BPE" == *"src"* ]]; then
    ...
    $ONMT/tools/learn_bpe.py -s $BPE_SRC_OPS < $TRAIN_SRC > $OUT/bpe-codes.src

    $ONMT/tools/apply_bpe.py -c $OUT/bpe-codes.src <  $TRAIN_SRC > $OUT/train.src
    $ONMT/tools/apply_bpe.py -c $OUT/bpe-codes.src <  $VALID_SRC > $OUT/valid.src
    $ONMT/tools/apply_bpe.py -c $OUT/bpe-codes.src <  $TEST_SRC > $OUT/test.src
else
    ln -sf $TRAIN_SRC $OUT/train.src
    ln -sf $VALID_SRC $OUT/valid.src
